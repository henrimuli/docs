...
Approvers receive email notifications when a new job is submitted for their review. The email includes a direct link to the Library card so they can approve or reject without navigating manually. To configure which email address receives approval notifications, visit your workspace notification settings.

Members who submitted a job receive an email when the job is approved or rejected, so they know when to act.
