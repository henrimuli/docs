...
CSV export is unlocked on **Creator** and **Business** plans. If you are on the Free plan, the export button is visible but disabled — upgrade to Creator to enable it.
...
<Steps>
  ...
  <Step title="Click Refresh">
    Select the **Refresh** button in the Analytics header. Hivra queues an ingestion request to each connected platform's API.
  </Step>
...
<AccordionGroup>
  <Accordion title="Grey coverage indicator for an account">
    A grey indicator next to an account name means Hivra does not have valid API access for that account. This is usually caused by an expired OAuth token. Go to **Integrations** and reconnect the affected account.
  </Accordion>
